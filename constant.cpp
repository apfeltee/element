...
namespace element
{
    Constant::Constant() : type(CT_Nil), integer(0)
    {
    }

    Constant::Constant(bool boolean) : type(CT_Bool), boolean(boolean)
    {
    }

    Constant::Constant(int integer) : type(CT_Integer), integer(integer)
    {
    }

    Constant::Constant(float floatingPoint) : type(CT_Float), floatingPoint(floatingPoint)
    {
    }

    Constant::Constant(const std::string& string) : type(CT_String), string(new std::string(string))
    {
    }

    Constant::Constant(CodeObject* codeObject) : type(CT_CodeObject), codeObject(codeObject)
    {
    }
    ...
    unsigned Constant::getSize() const
    {
        switch(type)
        {
            case CT_Nil:
            ...
            case CT_CodeObject:
            {
                ...
                return sizeof(Constant::Type) + 3 * sizeof(unsigned) + 2 * sizeof(int) + closureSize * sizeof(int)
                       + instructionsCount * sizeof(Instruction) + linesCount * sizeof(SourceCodeLine);
            }
        }
    ...
    char* Constant::writeConst(char* memoryDestination) const
    {
        switch(type)
        {
            case CT_Nil:
    ...
    char* Constant::readConst(char* memorySource)
    {
        clear();
    ...
    std::string Constant::toString() const
    {
        ...
        switch(type)
        {
            ...
            case CT_CodeObject:
            {
                ...
                result << "function - " << codeObject->localVariablesCount << " locals ("
                       << codeObject->namedParametersCount << " parameters)\n";
                ...
                for(unsigned i = 0; i < instructionsSize; ++i)
                {
                    ...
                    {
                        result << "       " << std::setw(3) << codeObject->instructionLines[linesIndex].line << " "
                               << std::setw(5) << i << " ";
                        ++linesIndex;
                    }
                    else
